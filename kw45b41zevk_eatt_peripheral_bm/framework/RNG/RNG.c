...
#if ((defined(USE_RTOS) && (USE_RTOS > 0)) && gSecLibUseMutex_c)
#define gRngUseMutex_c TRUE
#else
#define gRngUseMutex_c FALSE
#endif
#endif
...
#if gRngUseMutex_c
extern volatile uint32_t mSecLibMutexId[];
#define RNG_MUTEX_LOCK()   OSA_MutexLock((osa_mutex_handle_t)mSecLibMutexId, osaWaitForever_c)
#define RNG_MUTEX_UNLOCK() OSA_MutexUnlock((osa_mutex_handle_t)mSecLibMutexId)
#else
...
#if gRngUseMutex_c
        if (*mSecLibMutexId == 0U)
        {
            /*! Initialize the SecLib Mutex here. If not already done by SecLib module */
