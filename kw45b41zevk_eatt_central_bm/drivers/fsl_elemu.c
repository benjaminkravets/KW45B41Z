...
static const uint8_t s3fw[] = {
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
#else
#error "No ELEMU firmware available for current configuration!"
#endif
#endif
/* ELEMU driver */
...
status_t ELEMU_loadFw(ELEMU_Type *mu, uint32_t image[])
{
    ...
    do
    {
        ...
        /* OPEN SESSION */
        if (sss_sscp_open_session(&sssSession, kType_SSS_Elemu300, &sscpContext, 0u, NULL) != kStatus_SSS_Success)
        {
            break;
        }
        /* FIRMWARE UPLOAD */
...
{
    return ELEMU_loadFw(mu, (uint32_t *)s3fw);
}
